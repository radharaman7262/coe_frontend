...
import { VISUAL_ATTENTION_QUESTIONS, SCALE_OPTIONS } from './constant';

import { modulationMovementEmotionalProcessing } from './utils.api';
...
const ModulationMovementEmotionalProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentModulationEmo, setCommentModulationEmo] = useState('');
    ...
    const isFormComplete = VISUAL_ATTENTION_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: modulationMovementEmotionalProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = VISUAL_ATTENTION_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentModulationEmo },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentModulationEmo(response[0]?.data?.comments || '');
        }
    }, [response]);
    ...
    return (
        <div className={styles.container}>
            ...
            >
                {VISUAL_ATTENTION_QUESTIONS.map((q) => {
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        value={commentModulationEmo}
                        name='additionalComments'
                        onChange={(e) => setCommentModulationEmo(e.target.value)}
...
export default ModulationMovementEmotionalProcessing;
