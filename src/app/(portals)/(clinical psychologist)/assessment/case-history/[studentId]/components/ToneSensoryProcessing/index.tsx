...
import { SENSORY_TONE_QUESTIONS, SCALE_OPTIONS } from './constant';

import { sensoryProcessingTone } from './utils.api';
...
const SensoryProcessingTone = () => {
    ...
    const isFormComplete = SENSORY_TONE_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: sensoryProcessingTone,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = SENSORY_TONE_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentTone },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentTone(response[0]?.data?.comments || '');
        }
    }, [response]);
...
export default SensoryProcessingTone;
