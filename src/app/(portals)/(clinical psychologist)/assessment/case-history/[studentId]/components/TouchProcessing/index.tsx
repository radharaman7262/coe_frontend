...
import { TACTILE_QUESTIONS, SCALE_OPTIONS } from './constant';

import { touchProcessing } from './utils.api';
...
const TouchProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentTouch, setCommentTouch] = useState('');
    ...
    const isFormComplete = TACTILE_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: touchProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = TACTILE_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentTouch },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentTouch(response[0]?.data?.comments || '');
        }
    }, [response]);
            ...
            >
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        onChange={(e) => setCommentTouch(e.target.value)}
...
export default TouchProcessing;
