...
import { ORAL_SENSORY_QUESTIONS, SCALE_OPTIONS } from './constant';

import { oralSensoryProcessing } from './utils.api';
...
const OralSensoryProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentOral, setCommentOral] = useState('');
    ...
    const isFormComplete = ORAL_SENSORY_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: oralSensoryProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = ORAL_SENSORY_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentOral },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentOral(response[0]?.data?.comments || '');
        }
    }, [response]);
            ...
            >
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        onChange={(e) => setCommentOral(e.target.value)}
...
export default OralSensoryProcessing;
