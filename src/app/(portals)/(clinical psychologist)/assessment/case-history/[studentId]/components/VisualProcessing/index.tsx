...
import { VISUAL_QUESTIONS, SCALE_OPTIONS } from './constant';

import { visualProcessing } from './utils.api';
...
const VisualProcessing = () => {
    ...
    const [commentVisual, setCommentVisual] = useState('');
    ...
    const isFormComplete = VISUAL_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: visualProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = VISUAL_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentVisual },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentVisual(response[0]?.data?.comments || '');
        }
    }, [response]);
            ...
            >
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        onChange={(e) => setCommentVisual(e.target.value)}
