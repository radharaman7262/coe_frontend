...
import { VESTIBULAR_QUESTIONS, SCALE_OPTIONS } from './constant';

import { vestibularProcessing } from './utils.api';
...
const VestibularProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentVestibular, setCommentVestibular] = useState('');
    ...
    const isFormComplete = VESTIBULAR_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: vestibularProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = VESTIBULAR_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentVestibular },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentVestibular(response[0]?.data?.comments || '');
        }
    }, [response]);
            ...
            >
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        value={commentVestibular}
                        name='additionalComments'
                        onChange={(e) => setCommentVestibular(e.target.value)}
...
export default VestibularProcessing;
