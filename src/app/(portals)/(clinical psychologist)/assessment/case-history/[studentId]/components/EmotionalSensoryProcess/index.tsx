...
import { emotionalSensoryProcessing } from './utils.api';

import { EMOTIONAL_REGULATION_QUESTIONS, SCALE_OPTIONS } from './constant';
...
const EmotionalProcessing = () => {
    ...
    const [commentEmotional, setCommentEmotional] = useState('');
    ...
    const isFormComplete = EMOTIONAL_REGULATION_QUESTIONS.every(
        (q) => formData[q.key] !== undefined,
    );

    const mutation = useAppMutation({
        mutationFn: emotionalSensoryProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = EMOTIONAL_REGULATION_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentEmotional },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentEmotional(response[0]?.data?.comments || '');
        }
    }, [response]);
    ...
    return (
        <div className={styles.container}>
            ...
            >
                {EMOTIONAL_REGULATION_QUESTIONS.map((q) => {
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        value={commentEmotional}
                        name='additionalComments'
                        onChange={(e) => setCommentEmotional(e.target.value)}
