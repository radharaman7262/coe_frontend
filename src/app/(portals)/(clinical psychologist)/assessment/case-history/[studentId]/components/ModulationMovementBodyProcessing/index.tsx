...
import { ACTIVITY_LEVEL_QUESTIONS, SCALE_OPTIONS } from './constant';

import { modulationMovementProcessing } from './utils.api';
...
const ModulationMovementProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentModulationMovement, setCommentModulationMovement] = useState('');
    ...
    const isFormComplete = ACTIVITY_LEVEL_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: modulationMovementProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = ACTIVITY_LEVEL_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentModulationMovement },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentModulationMovement(response[0]?.data?.comments || '');
        }
    }, [response]);
    ...
    return (
        <div className={styles.container}>
            ...
            >
                {ACTIVITY_LEVEL_QUESTIONS.map((q) => {
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        value={commentModulationMovement}
                        name='additionalComments'
                        onChange={(e) => setCommentModulationMovement(e.target.value)}
...
export default ModulationMovementProcessing;
