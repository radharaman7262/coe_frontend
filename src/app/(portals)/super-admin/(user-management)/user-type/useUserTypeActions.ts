...
import { UserTypeMutationPayload, useUserTypeMutation } from './_components/mutation';

export const useUserTypeActions = ({
    router,
    setLoader,
    setShow,
}: {
    ...
    const mutation = useUserTypeMutation();

    const execute = async (payload: UserTypeMutationPayload) => {
        try {
            setLoader(true);
