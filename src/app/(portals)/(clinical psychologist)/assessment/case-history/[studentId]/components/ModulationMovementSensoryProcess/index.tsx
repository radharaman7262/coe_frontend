...
import { SOCIAL_EMOTIONAL_QUESTIONS, SCALE_OPTIONS } from './constant';

import { modulationMovementSensoryProcessing } from './utils.api';
...
const ModulationMovementSensoryProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentSensory, setCommentSensory] = useState('');
    ...
    const isFormComplete = SOCIAL_EMOTIONAL_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: modulationMovementSensoryProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = SOCIAL_EMOTIONAL_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentSensory },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentSensory(response[0]?.data?.comments || '');
        }
    }, [response]);
    ...
    return (
        <div className={styles.container}>
            ...
            >
                {SOCIAL_EMOTIONAL_QUESTIONS.map((q) => {
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        value={commentSensory}
                        name='additionalComments'
                        onChange={(e) => setCommentSensory(e.target.value)}
...
export default ModulationMovementSensoryProcessing;
