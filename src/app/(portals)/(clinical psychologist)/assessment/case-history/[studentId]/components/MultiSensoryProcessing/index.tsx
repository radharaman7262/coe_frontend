...
import { multiSensoryProcessing } from './utils.api';

import { MULTI_SENSORY_QUESTIONS, SCALE_OPTIONS } from './constant';
...
const MultiSensoryProcessing = () => {
    const [formData, setFormData] = useState<FormState>({});
    const [commentMultiSensory, setCommentMultiSensory] = useState('');
    ...
    const isFormComplete = MULTI_SENSORY_QUESTIONS.every((q) => formData[q.key] !== undefined);

    const mutation = useAppMutation({
        mutationFn: multiSensoryProcessing,
        setLoader,
    ...
    const handleSubmit = () => {
        const rawData = MULTI_SENSORY_QUESTIONS.map((q) => ({
            questionId: q.id,
            key: q.key,
        ...
        const payload = {
            data: { details: rawData, comments: commentMultiSensory },
            studentId,
        };
    ...
    useEffect(() => {
        if (response?.length) {
            ...
            setCommentMultiSensory(response[0]?.data?.comments || '');
        }
    }, [response]);
            ...
            >
                ...
                <div className={styles.comments}>
                    ...
                    <Input
                        ...
                        value={commentMultiSensory}
                        name='additionalComments'
                        onChange={(e) => setCommentMultiSensory(e.target.value)}
...
export default MultiSensoryProcessing;
